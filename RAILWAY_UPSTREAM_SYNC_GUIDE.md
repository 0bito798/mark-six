...
### 9. APP 地区选择持久化 (涉及文件：`mobile/mark_six/lib/main.dart`)
上游 Flutter APP 的多个页面会把地区状态直接初始化为 `String _region = 'hk';`，用户切换到澳门后只保存在当前页面内存中。重新进入 APP、切换页面导致页面重建、或重新打开预测/开奖记录/手动选号页面时，会再次默认香港。
- 在 `main.dart` 中使用项目已有的 `shared_preferences` 依赖新增统一偏好工具，例如：
  - `const String _preferredRegionPrefsKey = 'preferred_region';`
  - `loadPreferredRegion()`：读取 `SharedPreferences`，只允许 `hk` / `macau`，非法值回退 `hk`。
  - `savePreferredRegion(region)`：用户切换地区时写入同一个 key。
- 检查所有包含地区选择的页面，至少包括 `ManualPickScreen`、`RecordsScreen`、`PredictScreen`：
  - `initState()` 不要直接用默认 `hk` 请求数据，应先异步调用 `loadPreferredRegion()`，恢复 `_region` 后再加载开奖记录、下注记录、预测记录等数据。
  - 所有地区切换按钮或 `SegmentedButton` 的 `onTap` / `onSelectionChanged` 中，除了 `setState(() => _region = value)`，还必须调用 `savePreferredRegion(value)`。
- 修改后全局搜索 `String _region = 'hk'`、`_region = value`、`SegmentedButton`，确认每个地区切换入口都能保存偏好；重新编译 APP 后验证：切换澳门 → 退出/重进 APP → 默认仍为澳门。
